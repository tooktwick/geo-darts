# Geo Darts Japan (ジオダーツ 日本列島) システム仕様書

**文書バージョン**: 1.4.0  
**改訂日**: 2026年9月23日  
**対象システム**: Geo Darts Japan (ジオダーツ 日本列島) Webアプリケーション
...
### 1.1 システムの目的・コンセプト
『Geo Darts Japan（ジオダーツ 日本列島）』は、日本全国47都道府県および**全国940箇所の厳選観光名所・史跡・絶景**を舞台に、地理知識・精密な狙撃力・風の読みを融合させた和風モダン・ブラウザ地理ダーツゲームです。
...
- **名所画像の大幅拡充 & マルチバリエーションシステム (`landmarkImages.ts`)**:
  - **全国47都道府県・主要名所写真（130箇所以上）**: 各県の代表的観光地・景勝地（富士山、金閣寺、厳島神社、白川郷、高千穂峡、姫路城、兼六園、美ら海、松本城、阿蘇山等）に実写高解像度写真を完備。
  - **カテゴリ別マルチバリエーション画像プール (`CATEGORY_IMAGE_VARIANTS`)**: 各カテゴリに4〜5種類の美麗写真を用意。個別指定がない名所でも、名所IDのハッシュ値による決定論的選択により、同じカテゴリ内でも多種多様な風景・写真が表示される「種類増」を実現。
  - **オフライン安全フォールバック**: ネットワーク瞬断時や画像ロード失敗時（`onError`）は、即座にローカルの高品質カテゴリ画像（`/assets/categories/category_*.jpg`）へ自動切り替えする二重防御機構を配備。
